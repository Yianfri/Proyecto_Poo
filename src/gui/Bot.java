...
public class Bot extends Parent {
	...
	private void stillAlive() {
		...
		Cell randomCell = Main.playerBoard.getCell(firstHit.getXCord(), firstHit.getYCord());
		while (hitFlag) {
	...
	private void notAlive() {
		...
		randomCell = Main.playerBoard.getCell(randomCell.getXCord(), randomCell.getYCord());
		firstHit = Main.playerBoard.getCell(randomCell.getXCord(), randomCell.getYCord());
		firstHit=randomCell;
	...
	private void gameOverCheck() {
		// TODO Auto-generated method stub
		if (Main.playerBoard.ships <= 0) {
			Alert alert = new Alert(AlertType.INFORMATION);
			alert.setTitle("Resultado Final");
			alert.setHeaderText("Has perdido, total de movimientos: "+Main.moveCounter);
	...
	private Cell goUp(Cell c) {
		...
		c = new Cell(x, y);
		c = Main.playerBoard.getCell(x, --y);
		while (c.wasShot) {
			c = getRandomCell();
			c = Main.playerBoard.getCell(c.getXCord(), c.getYCord());
